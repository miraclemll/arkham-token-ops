...
2. 可复用的 `token-control-monitor` skill，适合安装到 Codex / OpenClaw，用于项目方控筹分析、地址分析以及 macOS `launchd` 常驻监控

如果你的目标是快速接入单一代币监控，可以直接使用原始脚本。
如果你需要支持更多 token、进行地址画像分析，或希望安装 skill 后即可直接通过自然语言进行调用，建议优先使用 `skills/token-control-monitor/`。
...
| 项目方控筹分析 | `skills/token-control-monitor/scripts/token_control_monitor.py` | 分析任意 token 的项目方地址、持仓结构、地址画像以及近期或大额转账 |
| macOS 常驻监控 | `token-control-monitor monitor install` | 在 macOS 上通过 `launchd` 长期运行监控 |
| Codex / OpenClaw skill | `skills/token-control-monitor/` | 安装 skill 后即可直接通过自然语言进行调用 |
...
│       ├── SKILL.md
│       ├── agents/openai.yaml
│       ├── scripts/token_control_monitor.py
│       ├── scripts/lib/
│       └── references/
└── README.md
```
...
### 方案 B：使用 `token-control-monitor` skill

适合：

- 需要分析多个 token，而不局限于单一资产
- 需要持续跟踪项目方手中的代币数量和控筹比例
- 需要查看链上地址画像与资金流向
- 需要在 macOS 设备上长期运行监控
...
## Token Control Monitor Skill

`skills/token-control-monitor/` 是该仓库中更通用、也更适合复用的控筹分析能力层。
...
```bash
python3 skills/token-control-monitor/scripts/token_control_monitor.py --help
```
...
```bash
python3 skills/token-control-monitor/scripts/token_control_monitor.py \
  token-report \
  --chain ethereum \
...
```text
~/.codex/skills/token-control-monitor
```

如果环境中设置了自定义 `CODEX_HOME`，安装器会优先写入：

```text
$CODEX_HOME/skills/token-control-monitor
```
...
```bash
cp -R skills/token-control-monitor ~/.codex/skills/
```
...
```text
https://github.com/<owner>/<repo>/tree/<branch>/skills/token-control-monitor
```
...
- `Use $token-control-monitor to analyze a token on Ethereum`
- `Use $token-control-monitor to inspect this address on Ethereum: 0x...`
- `Use $token-control-monitor to deploy a token transfer monitor on macOS`

skill 说明文件位于：

- `skills/token-control-monitor/SKILL.md`

---

## macOS 常驻监控

`token-control-monitor` 默认使用 per-user `LaunchAgent`，适合部署到长期运行的 macOS 设备。
...
```bash
python3 skills/token-control-monitor/scripts/token_control_monitor.py monitor status --name token-main
```

### 查看日志

```bash
python3 skills/token-control-monitor/scripts/token_control_monitor.py monitor logs --name token-main
```

### 停止监控

```bash
python3 skills/token-control-monitor/scripts/token_control_monitor.py monitor stop --name token-main
```

### 删除监控

```bash
python3 skills/token-control-monitor/scripts/token_control_monitor.py monitor uninstall --name token-main
```
...
如果希望长期维护并扩展该项目，最推荐的入口是 `skills/token-control-monitor/`。原始 `monitor.py` 更适合作为面向单一目标代币的轻量脚本保留。
...
使用 `token-control-monitor` 的 `token-report`、`recent-transfers` 和 `monitor install`，将示例地址替换为目标 token 地址即可。
